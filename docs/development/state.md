...
> **Status**: Active | **Last Updated**: 2026-08-24 (v0.8.0 — **`--watch` anomaly stream; M3 CLOSED**. `shu --watch` tails aegis's append-only NDJSON event sink and renders events live. The design spec's premise ("aegis and phylax already publish events") was verified false before building — aegis 1.1.6 was a stub with zero write/socket syscalls — so the producer was fixed first in **aegis 1.1.7**. `--watch` ships in the **LEAN** `shu`, not `shu-ai`: tailing a local file needs no libc, no network and no new dep, so it works on a pure no-libc AGNOS host where `shu-ai` cannot run. Lean **116/116**, AI 21/21, smoke PASS (+5 watch gates), PTY **12/12**. End-to-end verified against a real aegis producer, and live-follow proven under a PTY.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.8.0** — `--watch` anomaly stream, closing **M3**. New `src/watch.cyr`: NDJSON field reader (no JSON parser linked — the lean build keeps bayan out), 64-slot event ring, tail reader handling truncation/rotation and partial records, plus a full-screen panel and a non-TTY plain dump. Consumes **aegis 1.1.7**'s `AEGIS_EVENT_LOG` sink. `--watch` now accumulates like `-p` instead of returning mid-loop, and rejects `-p`/`--pid` combinations with exit 2. |
