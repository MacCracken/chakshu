...
> **Status**: Active | **Last Updated**: 2026-08-24 (v0.9.1 — **GPU telemetry depth**. The gpu line now carries live `busy% / vram used-total / temperature` read straight from DRM sysfs (`/sys/class/drm/cardN/device/`), not just static identity. ai-hwaccel has **no** utilisation/temp/power surface at all, and a vendor library would mean FFI — but the kernel publishes these as plain text, which is chakshu's contract exactly. Devices are matched to mihi's list by **PCI id**, not index, so a non-telemetry card (NVIDIA) cannot be credited with another card's utilisation. Works on amdgpu; i915/xe and NVIDIA proprietary publish nothing and degrade silently. Lean suite 116 → **130**.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.9.1** — GPU telemetry depth. New `src/gpu.cyr` reads live `gpu_busy_percent`, `mem_info_vram_used/total` and hwmon `temp1_input` from DRM sysfs, replacing the static capacity in both `-p` and the TUI when available. Card↔accelerator pairing is by PCI id parsed from mihi's device name and matched against sysfs `vendor`/`device`. No new dependency — the syscalls were already in the lean manifest. |
