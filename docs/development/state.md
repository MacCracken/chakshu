...
> **Status**: Active | **Last Updated**: 2026-05-09 (v0.2.1 cut — M2 Slices A+B+C ship)
>
...
| Field | Value |
|-------|-------|
| Version | **0.2.1** — M2 in-progress checkpoint (Slices A+B+C); next minor (0.5.0) cuts at M2 close |
...
| M2 | Full TUI | **In progress** — Slices A (alt-screen + raw mode + q/Ctrl-C) + B (exit-signalfd cleanup) + C (1Hz render loop + `--rate`) + D (SIGWINCH + dynamic window size) + E (↑/↓ row selection + `s` cycles sort, with reverse-video highlight) ✓; Slices E.5 (filter), F (kill+confirm), G (--pid + color + PTY smoke + close) pending. Powered by **darshana 0.3.0** — chakshu has zero termios/ANSI code of its own. |
