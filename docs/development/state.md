...
| Field | Value |
|-------|-------|
| Version | **0.9.2** — theming. Colour escapes now route through a theme palette (`_tui_c_low/mid/high/idle/faint`) instead of being hardcoded at each site, so `--theme light` re-tints only what is unreadable on white. `--color auto` gained real detection (NO_COLOR / TERM / isatty(stdout)). Selection reverse-video is deliberately NOT gated by `--color never` — it is the only selection cue on a monochrome terminal. |
