...
> **Status**: Active | **Last Updated**: 2026-08-24 (v0.8.2 — **AI hardening**. The hoosh-stub smoke, the last `continue-on-error` step in either workflow, is now a **hard gate** — and the reason it never passed was not the reason recorded. The CI note blamed sandhi needing libc for DNS; verified false (sandhi has an IPv4-literal fast path and its own UDP resolver, and the POST returns 200 here). The real defect was chakshu's: `_ai_extract_content` searched for the literal `"content":"` and could not parse `"content": "..."` — Python json.dumps' default — and reported that parse miss as a **transport** error, so a 200 response printed "hoosh unreachable". Both fixed; **no step in either workflow is continue-on-error any more**. AI suite 30 → 39. **Also fixed a release-blocking gap**: `scripts/smoke.sh` hardcoded the lean build's `--with-logs` exit code, but `release.yml` runs that script against BOTH binaries while `ci.yml` only ran it against the lean one — so the assertion could not fail until a tag was pushed (it broke 0.8.1 and 0.8.2). The assertion is now build-aware and CI runs the shu-ai smoke too; CI is a superset of release.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.8.2** — AI hardening. `_ai_extract_content` now tolerates JSON whitespace around the colon (it was fixed-literal, so any pretty-printed or `json.dumps`-shaped response failed), and an extraction miss maps to -3 rather than -1, so a 200 response with an unexpected shape no longer prints "hoosh unreachable". `tests/hoosh_stub_smoke.py` promoted to a hard CI gate. The roadmap's runtime-libc fallback item is closed as **not needed** — it rested on the same false premise. |
