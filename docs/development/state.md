...
> **Status**: Active | **Last Updated**: 2026-08-24 (v0.7.14 cut — **dep-pin cut, no source change**: ai-hwaccel `2.3.18` → `2.3.19` and mihi `1.2.4` → `1.2.5` (both manifests), and `"bayan"` dropped from the lean `[deps].stdlib`. Closes the P-1 sweep's last open item: the lean binary had been linking all 641 KB of the bayan bundle for seven JSON calls it never reaches. **Lean `shu`: 861,536 → 571,480 B, −290,056 B (−33.7%)**; `shu-ai` 3,179,096 → 3,175,000 B. Design-spec §8 goes from ~3.4× to **~2.2× over**. Lean 81/81, AI 21/21, smoke PASS, PTY 11/11, DCE parity PASS.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.7.14** — **dependency-pin cut**, no chakshu source change. ai-hwaccel `2.3.18` → `2.3.19` (canonical `bayan_json_v_*` API + focused `bayan-json` dep) and mihi `1.2.4` → `1.2.5` (dropped its `bayan` cover) in both manifests; `"bayan"` removed from the lean `[deps].stdlib` (23 → 22). `ai/cyrius.cyml` gains a direct `[deps.bayan]` at the FULL bundle — the AI chain's `ws`/`sigil`/`tls` genuinely need the monolith, and declaring it directly overrides ai-hwaccel's transitive sublib pin so only one bayan lands. Lean 81/81, AI 21/21, smoke PASS, PTY 11/11, DCE parity PASS. |
