...
| M4 | Polish + perf | Not started. **Size work targets the lean monitor** (design-spec §8 `< 256 KB`, now ~3.3× over at 857 136 B). The 0.7.11 `shu-ai` bloat item is **closed** — cyrius 6.5.22 reclaimed sigil's banked crypto `.bss` and the AI build fell to ~3.02 MB, an accepted opt-in cost. The lean regression is now upstream-shaped too: cycc 6.5.16 emits every declared stdlib module and `CYRIUS_DCE=1` no longer removes anything, so a chakshu-side fix means trimming declared modules, not more DCE. Also carried: `src/tui.cyr` tears the signalfds down with bare `file_close` and never calls `tty_close_signalfd`, leaving HUP/INT/TERM and WINCH blocked between teardown and `syscall(60)` (bounded today — the process exits immediately after — but darshana 1.0.0's docstring names this exact two-signalfd pattern as its motivating case). |
