...
> **Status**: Active | **Last Updated**: 2026-09-08 (v0.10.2 — **a fabricated 59% CPU%, found by running four cores for the first time**. `MAP_U64_EMPTY` is 0, so keying the tick baseline by pid made AGNOS's pid 0 read its whole uptime as one window's delta; keys are biased by +1 and it is filed with cyrius. The harness had been single-vCPU throughout — the blindness this project attributed to agnos in a filing was its own — and now runs `-smp 4` with three regression guards. Cyrius 6.6.1, niyama 1.0.10.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.10.2** — pid-0 hashmap-sentinel fix (a plausible wrong CPU%), harness to `-smp 4`, nameless-row filter removed now agnos 1.57.1 fixed the idle park it worked around. Cyrius `6.6.1`, niyama `1.0.10`. `check.sh` 23 gates, `agnos_qemu.py` 31. |
| Cyrius toolchain pin | `6.6.1` (both manifests) — bumped 6.6.0 → 6.6.1 at v0.10.2, a patch on top of 6.6.0's breaking minor (Result/Option became the register-pair value form; the old dep set does not compile under it, so toolchain and deps must move together). 6.6.1 folds niyama **1.0.10**, matching chakshu's pin, so the tag still names what links. ⚠ `MAP_U64_EMPTY` is 0 — key 0 is the empty-slot sentinel and `map_u64_get_or` returns 0 rather than the caller's default for it; filed, and worked around with a +1 key bias. (History: 6.5.35 at v0.7.12; 6.5.41 at v0.9.6; 6.5.45 at v0.9.9; 6.6.0 at v0.10.0.) ⚠ The wrapper re-execs to the manifest pin but resolves `cycc` from `~/.cyrius/bin`, so use an isolated `CYRIUS_HOME` for any baseline measurement. |
