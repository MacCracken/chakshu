...
> **Status**: Active | **Last Updated**: 2026-09-07 (v0.10.1 — **the AGNOS delta line stops saying `n/a`**. Disk and network rates ship (both declined at v0.9.9 for untrustworthy counters, both fixed upstream since), and the aggregate `cpu:` is **derived** from the halt asymmetry between the pooled per-core band and the summed per-process ticks — so agnos needs no idle field, and we filed asking them not to build one. Verified with agnos's own counter-MOVED oracle.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.10.1** — AGNOS disk + network rates and a **derived** aggregate cpu% (`src/snap_agnos.cyr`), guarded on LBA size, NIC presence and process naming. `check.sh` 23 gates, `agnos_qemu.py` 27. |
