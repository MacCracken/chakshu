...
> **Status**: Active | **Last Updated**: 2026-09-02 (v0.9.7 — **the no-libc rule loses its exception**. `shu-ai` linked libssl through `lib/fdlopen.cyr`; that was stale (cyrius 6.1.21 made the native TLS stack the default backend) and load-bearing — roadmap.md cited it as a v1.0 blocker. `src/nolibc.cyr` hard-refuses it; `fdlopen`/`dynlib` are gone from the AI manifest. Both binaries are now statically linked, 0 `NEEDED`, pure-syscall, and **`shu-ai` builds for AGNOS** (2,880,824 B). Verified by capturing a real TLS 1.3 ClientHello. check.sh → 22 gates.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.9.7** — the no-libc rule loses its exception. `shu-ai` no longer links any `dlopen` machinery (`src/nolibc.cyr` refuses the libssl bridge; `fdlopen`/`dynlib` dropped from `ai/cyrius.cyml`, `random` added for `getrandom(2)` handshake entropy). Both binaries statically linked, 0 `NEEDED`, no interpreter. **`shu-ai` compiles for AGNOS for the first time**; `--with-logs` fails closed there because its path validator needs procfs to canonicalise (audit F1). One of roadmap.md's three AGNOS blockers is struck. |
