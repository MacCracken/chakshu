...
> **Status**: Active | **Last Updated**: 2026-08-24 (v0.7.15 — **TUI sampling rework + roadmap reset**. The TUI took TWO full `/proc` walks per frame (~562 per-pid stat reads/sec at 1 Hz) plus a 100 ms sleep *inside every render*, so each keypress cost 100 ms and two walks before repainting. Now **one walk per frame**, differenced against the previous frame via a swapping pair of tick maps, with the window being real elapsed time rather than a fixed 100 ms; repaints under 200 ms reuse cached figures. `shu -p` unchanged. §8 audit: memory **4.49 MB MET**, cold start **~1.6 ms MET**, CPU 0.650% → **0.549% — still over 0.5%**, and now measured to scale with visible rows (`--top 1` is 0.466%), so it is the per-row cmdline read and render, not the walk. Carried to 0.9.0. `roadmap.md` rewritten forward-only and batched into 0.8.0 → 1.0.0.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.7.15** — TUI sampling rework: one `/proc` walk per frame instead of two, rolling inter-frame delta baseline, per-frame 100 ms sleep removed, static identity facts cached. `shu -p` deliberately unchanged (a one-shot snapshot has no previous frame). Design-spec §8 audit run: memory and cold-start **MET**, CPU improved 15.5% but still over. `docs/development/roadmap.md` rewritten to be forward-looking only and re-batched into concrete releases (0.8.0 `--watch` → 1.0.0 ship). Lean 81/81, AI 21/21, smoke PASS, PTY 11/11, DCE parity PASS. |
...
| Test count | **Two suites**: monitor `tests/chakshu.tcyr` (**81**) + AI `ai/tests/chakshu-ai.tcyr` (**21**), plus the PTY suite at **11** scenarios. Grew 64→81 / 13→21 / 7→11 in the 0.7.13 sweep; every added assertion was checked non-vacuous by mutation (reverting its fix must fail it). TUI render path is still the least-covered surface. |
