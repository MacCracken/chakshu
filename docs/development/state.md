...
> **Status**: Active | **Last Updated**: 2026-09-07 (v0.10.0 — **CPU% ships on AGNOS**. agnos 1.56.60 added the halt exclusion chakshu asked for after v0.9.9 measured itself at 100% while asleep, so per-process ticks are real CPU time; chakshu's own row now reads 0%. Cyrius **6.6.0** is a breaking minor — the old dep set does not compile under it — so toolchain and deps moved together. Lean binary −8.3% (bayan left the closure), and `CYRIUS_DCE=1` prunes again: the shipped lean artifact is **399 KB**.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.10.0** — AGNOS CPU% (halt-excluded ticks / pooled `sysinfo` +40 band), Cyrius 6.6.0, deps to latest. Lean 663,704 → **608,328 B**; DCE'd **399,432 B**. `check.sh` 23 gates, `agnos_qemu.py` 25. |
| Cyrius toolchain pin | `6.6.0` (both manifests) — bumped 6.5.45 → 6.6.0 at v0.10.0. ⛔ **A BREAKING MINOR**: Result/Option/Either became the register-pair value form, deleting `payload()`/`tagged_new()` and adding a tag argument to ten helpers. chakshu's own source calls none of them, but **bayan 1.5.4 does**, so a control build of 6.6.0 against v0.9.9's dep tags FAILS outright (`a \`: stack\` enum returns two values — bind both`). Toolchain and deps must move together. Stdlib file list is otherwise identical (zero additions, zero removals) and every declared module resolves. ⭐ 6.6.0 also **restored DCE pruning** (a no-op since cycc 6.5.16) and shipped `proclist_cpu_ticks` / `proclist_rss_pages` — the accessors chakshu filed for at v0.9.9. (History: 6.5.35 at v0.7.12; 6.5.41 at v0.9.6; 6.5.45 at v0.9.9.) ⚠ The wrapper re-execs to the manifest pin but resolves `cycc` from `~/.cyrius/bin`, so use an isolated `CYRIUS_HOME` for any baseline measurement. |
| Genesis cycle | v6.6.x — toolchain rev adopted; both manifests pin 6.6.0 |
...
| Binary size | **Lean `shu`: 608,328 B** plain, **399,432 B DCE'd** — and `release.yml` ships the DCE'd one, so the download is now ~399 KB against design-spec §8's revised < 768 KB, within sight of the original 256 KB M0 figure long treated as unreachable. The −55,376 B plain drop is **not codegen**: ai-hwaccel 2.3.22 marks `[deps.bayan]` `optional = true`, so `bayan-json` left the lean closure (root lock 4 → 3 commit-pinned deps) and the AGNOS target fell by exactly the same count. AGNOS: **606,184 B**. `shu-ai`: **2,941,624 B** (+16,440 — bayan is still declared there and 1.5.5 is larger); DCE does not prune the AI build. |
...
- git deps: **darshana** `1.1.1` (TTY/raw-mode — chakshu has zero termios/ANSI of its own; v1.0 **API freeze**, so the 15 symbols chakshu uses are contractually stable and 1.x minors need no diff read), **mihi** `1.2.6` (identity probes: hostname/kernel/distro/cpu/mem/uptime/gpu_* via `lib/mihi.cyr`), **ai-hwaccel** `2.3.22` (transitive via mihi; GPU panel; pin tracks **mihi's own** tag, not ai-hwaccel latest — mihi 1.2.6 pins 2.3.22, so v0.10.0 moves with it — after holding at 2.3.19 for two cuts while mihi pinned that, which is the rule working rather than being suspended. **The two must move in lockstep**: 2.3.18 against mihi 1.2.1 compiles but leaks `detect: profiles=N` to stderr mid-frame).
...
- **bayan** `1.5.5` (AI manifest only, declared at the full bundle). ⚠ **cyrius 6.5.39 reversed stdlib-vs-git-dep precedence**: `cyrius deps` now keeps the toolchain's folded `lib/bayan.cyr` and SKIPS the pinned dep artifact, printing `refusing to overwrite stdlib leaf 'bayan'`. The tag therefore selects no bytes — it was moved 1.5.2 → 1.5.4 at v0.9.6 so it at least names what links (all three of `ai/lib/bayan.cyr`, the 6.5.41 stdlib copy and the 1.5.4 dep dist are now byte-identical, `md5 21e1dc32…`; at 1.5.2 a genuinely different file was being skipped). ⚠ **That stated reason expired at v0.10.0**: ai-hwaccel 2.3.22 marks its own `[deps.bayan]` `optional = true`, so there is no transitive sublib left to collide with — which is exactly why the lean binary shrank 55 KB. The block is retained because the AI build genuinely uses bayan through the ws/sigil/tls chain, not to suppress a collision that no longer happens.
