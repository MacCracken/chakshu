...
> **Status**: Active | **Last Updated**: 2026-08-24 (v0.9.0 — **perf + size close-out, opens M4**. All design-spec §8 targets now met. CPU at 1 Hz went 0.517% → **0.433–0.500%** (mean 0.473%, five 60 s windows) — and the cause was not what it looked like: caching the per-row `/proc/<pid>/cmdline` read moved the number *not at all* (79% hit rate, instrumented), while batching each table row from ~10 `write(2)` calls into **one** did. The §8 binary target was **revised 256 KB → 768 KB**: no chakshu-side lever remains (~25 KB of safe drops; `CYRIUS_DCE=1` is a parity check, not an optimizer, since cycc 6.5.16) and the rest is mihi/ai-hwaccel bundle bulk. Added `tests/MANUAL.md` for the checks that genuinely need human eyes.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.9.0** — perf + size close-out. Table rows are now composed into a buffer and emitted with one `write(2)` instead of ~10 (≈180 syscalls/frame at 18 rows); `/proc/<pid>/cmdline` is cached in a 64-slot direct-mapped cache keyed on pid **and comm**, so an exec or pid reuse invalidates it. design-spec §8's binary target revised 256 KB → 768 KB with the reasoning recorded in the spec. New `tests/MANUAL.md`. |
...
| Binary size | **Lean `shu`: 622,128 B** against design-spec §8's **revised < 768 KB** target — met, with headroom. (History: 861,536 B at 0.7.13 → 571,480 B at 0.7.14 once the bayan monolith left the closure → 622 KB after `--watch` and the 0.9.0 render work.) The old 256 KB figure was set at M0 before chakshu took its dependencies and is unreachable while the lean build links mihi + ai-hwaccel; ~25 KB of safe stdlib drops remain and `CYRIUS_DCE=1` emits a byte-identical binary, so the rest is upstream bundle bulk. **`shu-ai`: ~3.2 MB**, explicitly out of scope as the opt-in heavy build. |
...
| Test count | **Two suites**: monitor `tests/chakshu.tcyr` (**116**) + AI `ai/tests/chakshu-ai.tcyr` (**39**), plus the PTY suite at **12** scenarios. Grew 81→116 / 13→21 / 11→12 at v0.8.0 in the 0.7.13 sweep; every added assertion was checked non-vacuous by mutation (reverting its fix must fail it). TUI render path is still the least-covered surface. |
| Perf (§8, v0.9.0) | CPU at 1 Hz **0.433–0.500%** (mean 0.473%, n=5 x 60 s, 295 procs / 16 cores) vs `< 0.5%` — met at the margin; steady RSS **4.56 MB** vs `< 8 MB`; cold start **~2.0 ms** vs `< 5 ms`. |
