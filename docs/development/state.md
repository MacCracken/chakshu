...
> **Status**: Active | **Last Updated**: 2026-09-02 (v0.9.8 — **AGNOS grows a process table**. `#99 proclist` (agnos 1.56.47) has been reachable since cyrius **6.5.35** — the pin held *before* v0.9.6 — but nothing re-read the syscall table, so the "blocked upstream" note outlived its own fix. `src/proc_agnos.cyr` implements it; verified populated on a real kernel under QEMU. CPU%/MEM%/USER read `n/a`, never a fabricated 0. Second of three v1.0 AGNOS blockers struck.)
>
...
| Field | Value |
|-------|-------|
| Version | **0.9.8** — the AGNOS process table. `src/proc_agnos.cyr` fills the shared 56-byte record from `proclist`'s 64-byte snapshot, so `processes_sample1/2/_delta` dispatch under `#ifdef CYRIUS_TARGET_AGNOS` and **no call site changed**; the Linux path is byte-identical in behaviour. Untracked columns render `n/a`; `--sort cpu|mem|user` falls back to PID on AGNOS because all three rank on the same sentinel. |
