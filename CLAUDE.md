...
- **No libc, no FFI — in BOTH binaries.** `/proc` is plain text; the kernel ABI is the contract. **The v0.9.7 cut removed the last exception**: `shu-ai` used to reach libssl through `lib/fdlopen.cyr` (a real glibc `dlopen`), which is why the docs said it "only runs on a libc host". That was stale — cyrius 6.1.21 made the sovereign native TLS stack the default backend, and sandhi has its own UDP DNS resolver. `src/nolibc.cyr` hard-refuses the libssl path and `ai/cyrius.cyml` declares neither `fdlopen` nor `dynlib`, so both binaries are statically linked, pure-syscall, 0 `NEEDED`. ⛔ Do NOT re-declare `fdlopen`/`dynlib` to "fix" a TLS problem — that restores the libc bridge and the AGNOS regression with it; fix it in the native backend. Gated in `ci.yml` + `check.sh`. Keep the AI deps out of the root manifest (`cyrius.cyml`) — they live only in `ai/cyrius.cyml`.
